...
3. Attach an inline policy to it with a statement like this (swap in your real endpoint ID):
   ...
   **Important: attach this *alongside* your existing permissions, not instead of them.** A `Deny` statement never grants anything on its own — it only carves an exception out of permission that already exists elsewhere. If this ends up being the *only* policy on the identity (easy to do by accident if you're not careful in the console), every request will fail everywhere, including correctly through your endpoint, because there's no `Allow` left for the Deny to have any effect on. Make sure the identity still has whatever was granting it access before (the default managed policy on a Bedrock API key's user, or the inline policy from the callout in Part 6) attached *in addition to* this Deny.
